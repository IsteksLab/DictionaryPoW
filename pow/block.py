...
def propagate():
  ...
  static = (block["header"]["version"].to_bytes((block["header"]["version"].bit_length() + 7) // 8, "big") + bytes.fromhex(block["header"]["prevHash"]) + bytes.fromhex(block["header"]["merkleRoot"]) + block["header"]["timestamp"].to_bytes((block["header"]["timestamp"].bit_length() + 7) // 8, "big") + block["header"]["difficulty"].to_bytes((block["header"]["difficulty"].bit_length() + 7) // 8, "big") + block["header"]["nonce"].to_bytes((block["header"]["nonce"].bit_length() + 7) // 8, "big"))
